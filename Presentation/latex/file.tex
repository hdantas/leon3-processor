...
\begin{newblock}{Interface with the Processor}
...
\begin{newblock}{Signals and Constants}
...
\frame{\frametitle{Negative Operations}
Alternative Baugh Wooley
% \begin{columns}
% \begin{column}{0.45\textwidth}
\begin{itemize}
\item Negate some AND operations.
\item Add a constant vector at the end.
\end{itemize}
% \end{column}
% \begin{column}{0.45\textwidth}
\begin{figure}
\includegraphics[width=0.9\textwidth,height=0.5\textheight,keepaspectratio]{Figures/modified-bw.png}
\caption{}
\end{figure}
% \end{column}
% \end{columns}
...
\frame{\frametitle{VHDL statements}
\begin{itemize}
\item Feed inputs to FOR-GENERATES (with IF-GENERATES)
\item IF holdn is HIGH update add the two final numbers with Baugh-Wooley vector (for signed operations) (ready and nready are not used).
\item Update MULO.(result/ICC).
\end{itemize}
}
...
\frame{\frametitle{Divider State Machine}
\begin{figure}
...
\frame{\frametitle{Divider Diagram}
\begin{figure}
\includegraphics[width=0.9\textwidth,height=0.8\textheight,keepaspectratio]{Figures/DivisorDiagramSpecial.png}
\caption{}
\end{figure}
}

\frame{\frametitle{Signed and Unsigned Division}
\begin{itemize}
...
\frame{\frametitle{Divider Simulation}
\begin{figure}
...
P/f [\si[per-mode=symbol]{\W\per\MHz} ] & 0.03959 & 0.03987 & 0.08317
\end{tabular}
\end{table}
}
...
\section{Conclusion}
\frame{\frametitle{Conclusion - Comparison with Metrics (Div)}
...
\frame{\frametitle{Conclusion II - Comparison with Metrics (Div \& Mul)}
\begin{table}[H]
\centering
\begin{tabular}{cccc}
&Baseline & Modified & Improvements\\\midrule
